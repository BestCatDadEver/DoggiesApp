...
class NotificationsHelper(val context : Context) {
    ...
    private val NOTIFICATIONS_ID = 1



    fun createNotification(){
        createNotificationsChannel()
        //When the user taps on the notification it will redirect to MainActivity and it will create a new task or clear an existing task.
    ...
    //Creates the channel for this notification, if we have another notification we should create another channel for that one.
    private fun createNotificationsChannel(){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            val name = CHANNEL_ID
            val descriptionText = "Dogs channel description"
